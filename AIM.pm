...
Net::AIM - OO interface to the AIM TOC client protocol
...
The hash %Net::AIM::ERROR_MSGS contains english translations of all of
...
$ERROR = $Net::AIM::ERROR_MSGS{$IM_ERR};
...
%ERROR_MSGS = ( 0 => 'Success',
		1 => 'Net::AIM Error: Unknown',
		2 => 'Net::AIM Error: Incorrect Arguments',
		3 => 'Net::AIM Error: Exceeded Max Packet Length (1024)',
		4 => 'Net::AIM Error: Reading from server',
		5 => 'Net::AIM Error: Sending to server',
...
This section documents every member function of the Net::AIM class.

=head2 $Net::AIM->new()

This is the Net::AIM Constructor.
...
values are optional):
    ...
    'aim_agent' => agentname (max 200 char) (default 'Net-AIM vX' where 
...
Returns a blessed instantiation of Net::AIM.
...
sub new
{
    ...
    $args{'aim_agent'} ||= 'Net-AIM v' . $VERSION;
...
#
# These are the functions that the Net::AIM package user
...
# callback function will be called with the read data as the
# argument.  Will read as much as possible (up to 
# $MAX_PACKLENGTH) or, if ioctl is available, it will read the
# number of bytes returned by FIONREAD.
#
# returns undef on error
#
    my $imsg = shift @_;
...
C<$srv_sock = $aim-E<gt>srv_socket;>
...
order to facilitate such splitting, C<%Net::AIM::SERVER_MSG_ARGS> is
supplied.  For each valid server message,
C<$Net::AIM::SERVER_MSG_ARGS{$msg}> will return one less than the
proper number of splits to perform on the data coming from the server.
The intended use is such:

C<($msg, $rest) = split(/:/, $aim-E<gt>read_sflap_packet(), 2);>
C<@msg_args = split(/:/, $rest, $Net::AIM::SERVER_MSG_ARGS{$msg});>
...
C<$aim-E<gt>send_sflap_packet($SFLAP_TYPE_DATA, $message, 0, 0);>
...
C<$SFLAP_TYPE_DATA>
C<$SFLAP_TYPE_ERROR>
C<$SFLAP_TYPE_SIGNOFF>
C<$SFLAP_TYPE_KEEPALIVE>
...
See the file F<sampleclient.pl> for an example of how to interact with
this class.
...
F<sampleclient.pl>
    ...
    A sample client that demonstrates how this object should be used.
